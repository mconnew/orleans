...
namespace Orleans.Runtime
{
    ...
    internal sealed class Message
    {
        ...
        [NonSerialized]
        private CheapStopwatch _timeInterval;

        [NonSerialized]
        public readonly CheapStopwatch _timeSinceCreation = CheapStopwatch.StartNew();
        ...
        public ActivationAddress TargetAddress
        {
            get
            {
                if (_targetAddress.HasValue) return _targetAddress.Value;
                if (!TargetGrain.IsDefault)
                {
                    _targetAddress = ActivationAddress.GetAddress(TargetSilo, TargetGrain, TargetActivation);
                    return _targetAddress.Value;
                }
        ...
        {
            get => _sendingAddress ?? (_sendingAddress = ActivationAddress.GetAddress(SendingSilo, SendingGrain, SendingActivation)).Value;
            set
            {
        ...
        public void Start()
        {
            _timeInterval = CheapStopwatch.StartNew();
        }
