...
namespace Orleans.Streams
{
    internal class ImplicitStreamSubscriberTable
    {
        ...
        {
            _bindings = bindings;
            _cache = new Cache(MajorMinorVersion.Zero, new List<StreamSubscriberPredicate>());
        ...
        private Guid MakeSubscriptionGuid(GrainType grainType, StreamId streamId)
        {
            ...
            return SubscriptionMarker.MarkAsImplictSubscriptionId(new Guid((int)JenkinsHash.ComputeHash(grainType.ToString()), s1, s2, tail.ToArray()));
        }
