...
namespace Orleans.Configuration
{
    ...
    public class MemoryGrainStorageOptionsValidator : IConfigurationValidator
    {
        ...
        public void ValidateConfiguration()
        {
            ...
            if(this.options.InitStage < ServiceLifecycleStage.EnableGrainCalls)
                throw new OrleansConfigurationException(
                   $"Configuration for {nameof(MemoryGrainStorage)} {name} is invalid. {nameof(this.options.InitStage)} must be larger than {ServiceLifecycleStage.EnableGrainCalls} since " +
