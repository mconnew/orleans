...
namespace Orleans.Runtime
{
    internal class ActivationDirectory : IEnumerable<KeyValuePair<ActivationId, IGrainContext>>
    {
        ...
        public void PrintActivationDirectory()
        {
            ...
            {
                string stats = Utils.EnumerableToString(activations.Values.OfType<ActivationData>().OrderBy(act => act.Name), act => string.Format("++{0}", act.DumpStatus()), Environment.NewLine);
                if (stats.Length > 0)
                {
